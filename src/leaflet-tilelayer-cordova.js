...
L.TileLayer.Cordova = L.TileLayer.extend({
  initialize: function(url, options, success_callback) {
    ...
    if (!options.folder)
      throw "L.TileLayer.Cordova: missing required option: folder";
    if (!options.name)
      throw "L.TileLayer.Cordova: missing required option: name";
    ...
    window.requestFileSystem(
      ...
      function(fshandle) {
        if (myself.options.debug)
          console.log("requestFileSystem OK " + options.folder);
        ...
        myself.fshandle.root.getDirectory(
          ...
          function(dirhandle) {
            if (myself.options.debug)
              console.log("getDirectory OK " + options.folder);
          ...
          function(error) {
            if (myself.options.debug)
              console.log(
                "getDirectory failed (code " + error.code + ")" + options.folder
              );
  ...
  calculateXYZListFromPyramid: function(lat, lon, zmin, zmax) {
    ...
    for (z = zmin; z <= zmax; z++) {
      ...
      if (this.options.debug)
        console.log(
          "Calculate pyramid: Z " + z + " : " + "Radius of " + radius
        );
  ...
  downloadAndStoreTile: function(x, y, z, success_callback, error_callback) {
    ...
    if (myself.options.debug)
      console.log("Download " + sourceurl + " => " + filename);
